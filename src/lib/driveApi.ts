...
function getDemoFilesResponse(query = ''): DriveFileListResponse {
  ...
  return { files: filtered, nextPageToken: undefined };
}
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (retryAfterHeader) {
        ...
        delayMs = Math.min(expDelay + jitter, 30000);
      }
...
export async function deleteDriveFile(fileId: string): Promise<void> {
  ...
  await fetchWithBackoff<void>(url, { method: 'DELETE' });
}
...
export async function createDriveFile(
  ...
  if (!content) {
    const url = `${DRIVE_API_BASE}/files?supportsAllDrives=true&fields=id,name,mimeType,size,modifiedTime,createdTime,parents,webViewLink`;
    return fetchWithBackoff<DriveApiFile>(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function getDriveAbout(): Promise<DriveAboutResponse> {
  ...
  if (isDemoToken(token)) {
    return {
      user: { displayName: 'Demo Drive User', emailAddress: 'user@example.com' },
      storageQuota: {
        limit: '16106127360',
        usage: '4294967296',
...
export async function getDriveFile(
  ...
  const url = `${DRIVE_API_BASE}/files/${fileId}?supportsAllDrives=true&fields=${encodeURIComponent(fields)}`;
  return fetchWithBackoff<DriveApiFile>(url, { method: 'GET' });
}
...
export async function moveDriveFile(
  ...
  const previousParents =
    removeParents && removeParents.length > 0 ? removeParents.join(',') : 'root';
...
/**
 * Find an existing non-trashed folder by exact name (zero-spam helper).
 * Returns the first matching folder or null if none exists.
 */
export async function findFolderByName(name: string): Promise<DriveApiFile | null> {
  const token = getToken();
  if (isDemoToken(token)) {
    await sleep(100);
    return null;
  ...
  const url = `${DRIVE_API_BASE}/files?${params.toString()}`;
  const res = await fetchWithBackoff<DriveFileListResponse>(url, { method: 'GET' });
