...
export const DuplicateFinder: React.FC = () => {
  ...
  const handleMoveSelected = async () => {
    ...
    try {
      ...
      for (const fileId of ids) {
        try {
          ...
          if (!currentParents || currentParents.length === 0) {
            const meta = await getDriveFile(fileId, 'parents');
            currentParents = meta.parents || [];
          }
          ...
          await db.logAction(
            ...
            fileId,
            local?.name,
            `Moved to custom folder "${name}" (${folder.id})`
          );

          movedCount++;
        } catch (moveErr: any) {
          console.error(`Failed to move file ${fileId}:`, moveErr);
          errors.push(fileId);
        }
      }
      ...
      if (movedCount > 0) {
        showNotice(
          `Moved ${movedCount} file(s) into folder "${name}".${errors.length ? ` ${errors.length} failed.` : ''}`,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-neutral-800 pb-3 gap-3">
        <div className="flex items-center gap-2">
          <button
            ...
            aria-pressed={activeSubTab === 'exact'}
          >
          ...
          <button
            ...
            aria-pressed={activeSubTab === 'semantic'}
          >
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="text-neutral-400 text-[11px]">
              ...
              <span className="font-mono text-purple-300 font-bold">
                {Math.round(semanticThreshold * 100)}%
              </span>
            </span>
            <input
              type="range"
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 p-3 bg-neutral-900/60 border border-neutral-800 rounded-xl">
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-neutral-700 hover:border-amber-600/60 hover:bg-amber-950/30 text-neutral-300 transition"
                  aria-pressed={allSelected}
                  aria-label={allSelected ? 'Deselect all files' : 'Select all files'}
                >
                  {allSelected ? (
                    <CheckSquare className="w-3.5 h-3.5 text-amber-400" aria-hidden="true" />
                  ) : someSelected ? (
                    <CheckSquare className="w-3.5 h-3.5 text-amber-400/60" aria-hidden="true" />
                  ) : (
                    <Square className="w-3.5 h-3.5" aria-hidden="true" />
                  )}
                  <span>{allSelected ? 'Deselect All' : 'Select All'}</span>
                  {selectedIds.size > 0 && (
                    <span className="ml-1 text-[10px] font-mono text-amber-300">
                      ({selectedIds.size})
                    </span>
                  )}
                </button>
                ...
                <button
                  ...
                  disabled={selectedIds.size === 0 || isMoving || isProcessingAction}
                  className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-sky-700/90 hover:bg-sky-600 disabled:opacity-40 text-white transition"
                  aria-label="Move selected files to a custom folder"
                >
                  {isMoving ? (
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1.5 text-xs text-neutral-400">
                  ...
                  <select
                    ...
                    className="bg-neutral-950 border border-neutral-700 rounded-lg px-2 py-1.5 text-xs text-neutral-200 focus:outline-none focus:ring-1 focus:ring-amber-500"
                  >
          ...
          {(!summary || displayedExactGroups.length === 0) ? (
            <div className="p-10 border border-dashed border-neutral-800 rounded-2xl text-center space-y-2 bg-neutral-900/20">
              ...
              <p className="text-xs text-neutral-500 max-w-sm mx-auto">
                {summary && summary.exactGroups.length > 0
                  ? 'Try adjusting the file type or size filters.'
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-neutral-800/80 pb-2.5">
                    ...
                    <button
                      ...
                      disabled={isProcessingAction || isMoving}
                      className="flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg bg-rose-950/60 border border-rose-800/60 text-rose-300 hover:bg-rose-900/60 transition disabled:opacity-40"
                      aria-label={`Trash ${group.files.length - 1} redundant copies`}
                    >
                      <Trash2 className="w-3.5 h-3.5" aria-hidden="true" />
                  ...
                  <div className="divide-y divide-neutral-800/40 text-xs">
                    {group.files.map((file, fIdx) => {
                      ...
                      const matchesCurrentFilters =
                        matchesFileType(file, fileTypeFilter) && matchesSize(file, sizeFilter);

                      return (
                        <div
                          key={file.id}
                          className={`py-2.5 flex items-center justify-between gap-2 px-2 rounded-lg transition ${
                            isSelected ? 'bg-amber-950/30' : 'hover:bg-neutral-800/20'
                          } ${!matchesCurrentFilters ? 'opacity-50' : ''}`}
                        >
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <input
                              ...
                              className="w-4 h-4 rounded border-neutral-600 bg-neutral-900 text-amber-500 focus:ring-amber-500 focus:ring-offset-0 cursor-pointer shrink-0"
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <a
                              href={getWebViewLink(file)}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 text-[11px] font-medium text-sky-400 hover:text-sky-300 transition px-2 py-1 rounded-md hover:bg-sky-950/40"
        ...
        <div className="space-y-4">
          {(!summary || summary.semanticPairs.length === 0) ? (
            <div className="p-10 border border-dashed border-neutral-800 rounded-2xl text-center space-y-2 bg-neutral-900/20">
              ...
              <p className="text-xs text-neutral-500 max-w-sm mx-auto">
                No Google Workspace documents exceeded the similarity threshold of{' '}
                {Math.round(semanticThreshold * 100)}%.
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs pt-1">
                    <div className="p-3 bg-neutral-950/80 border border-neutral-800 rounded-xl space-y-2">
                      ...
                      <div className="flex justify-between items-center pt-1">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-lg border border-neutral-700 hover:border-rose-700 hover:bg-rose-950/40 text-neutral-400 hover:text-rose-300 transition disabled:opacity-40"
                        >
                    ...
                    <div className="p-3 bg-neutral-950/80 border border-neutral-800 rounded-xl space-y-2">
                      ...
                      <div className="flex justify-between items-center pt-1">
                        <a
                          href={getWebViewLink(pair.fileB)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 text-[11px] text-sky-400 hover:text-sky-300"
                          aria-label={`Open ${pair.fileB.name}`}
                        >
                          <ExternalLink className="w-3 h-3" aria-hidden="true" />
                          Show
                        </a>
        ...
        >
          <div className="w-full max-w-md bg-neutral-900 border border-neutral-700 rounded-2xl shadow-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 id="move-modal-title" className="text-sm font-semibold text-neutral-100">
                Move {selectedIds.size} file(s) to a custom folder
              </h3>
              <button
                type="button"
                onClick={() => !isMoving && setShowMoveModal(false)}
                disabled={isMoving}
                className="p-1 rounded-lg text-neutral-400 hover:text-white hover:bg-neutral-800 transition disabled:opacity-40"
            ...
            <p className="text-xs text-neutral-400">
              A new folder will be created in your Google Drive root, and the selected files will be
              moved into it.
            </p>

            <div>
              <label htmlFor="folder-name" className="block text-xs font-medium text-neutral-300 mb-1.5">
                Folder name
              </label>
              <input
                id="folder-name"
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !isMoving) handleMoveSelected();
                }}
                placeholder="e.g. Duplicates Review 2026"
                disabled={isMoving}
                className="w-full px-3 py-2 text-sm bg-neutral-950 border border-neutral-700 rounded-xl text-neutral-100 placeholder:text-neutral-600 focus:outline-none focus:ring-2 focus:ring-sky-500/60 disabled:opacity-50"
                autoFocus
                aria-invalid={!!folderNameError}
                aria-describedby={folderNameError ? 'folder-error' : undefined}
              />
              {folderNameError && (
                <p id="folder-error" className="mt-1.5 text-xs text-rose-400" role="alert">
                  {folderNameError}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              <button
                ...
                disabled={isMoving}
                className="px-4 py-2 text-xs font-medium rounded-xl border border-neutral-700 text-neutral-300 hover:bg-neutral-800 transition disabled:opacity-40"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isMoving || !folderNameInput.trim()}
                className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-xl bg-sky-600 hover:bg-sky-500 text-white transition disabled:opacity-40"
              >
                {isMoving ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" aria-hidden="true" />
